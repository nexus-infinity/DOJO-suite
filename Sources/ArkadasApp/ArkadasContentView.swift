/*
 ...
 Frequencies: 717 Hz (Arkadaş / companion) · 963 Hz (Obi-Wan / observer) · 852 Hz (AI Mind / king's chamber)
 ...
 Design philosophy:
   ...
   - No "This is Arkadaş speaking", no "Roger that", no radio protocol.
...
// MARK: - ArkadasContentView

public struct ArkadasContentView: View {
            ...
            ScrollView(.vertical, showsIndicators: true) {
                LazyVStack(alignment: .leading, spacing: 10) {
                    ForEach(engine.messages) { message in
                        ArkadasMessageRow(message: message)
                            .id(message.id)
                    }
                    if engine.isProcessing {
                        ArkadasTypingIndicator(character: engine.activeCharacter ?? .arkadas)
                            .id("__typing__")
                    }
                }
                .padding(.horizontal, 16)
