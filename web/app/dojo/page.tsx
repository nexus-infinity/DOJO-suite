...
// Four-mode interface: Chat | Code | Memory | Foreman (Comms)
...
import { ForemanPanel } from '@/components/field/ForemanPanel'
...
export default function DOJOPage() {
  ...
  useEffect(() => {
    ...
    setMode(activeSession.mode === 'code' ? 'chat' : activeSession.mode)
  }, [activeSession])
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-surface shrink-0">
          ...
          <nav className="flex gap-1 bg-raised rounded-lg p-1">
            {(['chat', 'collab', 'foreman'] as const).map(nextMode => (
              <button
                key={nextMode}
                onClick={() => {
                  setMode(nextMode)
                  // Foreman is a channel surface, not a session mode — don't persist to session record
                  if (nextMode !== 'foreman') {
                    updateSessionLocally(activeSession.id, session => ({ ...session, mode: nextMode, updatedAt: new Date().toISOString() }))
                    persistSessionPatch(activeSession.id, { mode: nextMode })
                  }
              ...
              >
                {nextMode === 'chat' ? '◈ Chat' : nextMode === 'collab' ? '◫ Memory' : '⊗ Comms'}
              </button>
            ))}
          </nav>
